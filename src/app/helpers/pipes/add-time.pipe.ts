...
export class AddTimePipe implements PipeTransform {
  ...
  transform(dateString: string): string {
    ...
    if (diffInMinutes < 1) {
      ...
      return `${diffInMinutes} ${this.declineWord(diffInMinutes, 'минута', 'минуты', 'минут')} назад`;
    } else if (diffInHours === 1) {
      return '1 час назад';
    } else if (diffInHours < 24) {
      return `${diffInHours} ${this.declineWord(diffInHours, 'час', 'часа', 'часов')} назад`;
    } else if (diffInDays === 1) {
      return '1 день назад';
    } else if (diffInDays < 30) {
      return `${diffInDays} ${this.declineWord(diffInDays, 'день', 'дня', 'дней')} назад`;
